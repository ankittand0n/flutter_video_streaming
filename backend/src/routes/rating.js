...
router.post('/', auth, validate('rating'), async (req, res) => {
  try {
    ...
    if (rating) {
      ...
      await rating.updateRating(req.body.rating, req.body.review, req.body.title);
      rating = await Rating.findById(rating._id).populate('userId', 'username profile.name profile.avatar');
    } else {
      // Create new rating
      rating = new Rating({
        userId: req.user._id,
...
router.get('/content/:contentId', validateQuery('pagination'), async (req, res) => {
  try {
    ...
    // Get ratings
    const ratings = await Rating.getContentRatings(contentId, contentType, {
      rating: rating ? parseInt(rating) : undefined,
      spoiler: spoiler !== undefined ? spoiler === 'true' : undefined,
